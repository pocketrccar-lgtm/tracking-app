    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">
            <Link href={`/vendors/${product.vendor.id}`} className="text-emerald-600 hover:underline">
              {product.vendor.name}
            </Link>
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            ...
            <Badge variant="outline" className={product.inStock ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Card>
          <CardContent className="p-4">
            <div className="text-xs uppercase tracking-wider text-slate-500">Wholesale</div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-xs uppercase tracking-wider text-slate-500">Retail</div>
            <div className="mt-1 text-3xl font-bold font-mono text-slate-900">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-xs uppercase tracking-wider text-slate-500">MOQ</div>
      ...
      {product.notes && (
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-slate-700 whitespace-pre-line">{product.notes}</p>
          </CardContent>
        </Card>
      )}
              ...
              <TableBody>
                {product.priceHistory.map((h) => (
                  <TableRow key={h.id}>
                    ...
                    <TableCell className="text-xs text-slate-500">{h.recordedBy ?? "—"}</TableCell>
            ...
            <Table>
              ...
              <TableBody>
                {product.purchaseOrders.map((po) => (
                  <TableRow key={po.id}>
                    <TableCell>
                      <Link href={`/purchase-orders/${po.id}`} className="text-xs hover:text-emerald-600">
                        #{po.id.slice(-6)}
                      </Link>
                    </TableCell>
